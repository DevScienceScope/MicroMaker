...
{
public:
	Adafruit_SSD1306(uint8_t rawHeight = 32, uint8_t rawWidth = 128 , uint8_t orentation = 0)
		: Adafruit_GFX(rawWidth,rawHeight)
	{
		buffer.resize(rawHeight * rawWidth / 8);
	};

	void begin(uint8_t switchvcc = SSD1306_SWITCHCAPVCC, uint8_t orentation = 0);
...
public:
	 ...
	 */
	Adafruit_SSD1306_I2c(MicroBitI2C i2c, uint8_t i2cAddress = SSD_I2C_ADDRESS, uint8_t rawHeight = 32, uint8_t rawWidth = 128, uint8_t orentation = 0)
